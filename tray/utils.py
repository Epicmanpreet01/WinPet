...
def initialize_active_asset():
  ...
  assets = sorted(
    [
      name.strip() for name in os.listdir(config.LIBRARY_PATH)
  ...
  if (
    config.ACTIVE_SEQUENCE_NAME
    and config.ACTIVE_SEQUENCE_NAME in assets
  ):
  ...
  config.ACTIVE_SEQUENCE_PATH = os.path.join(config.LIBRARY_PATH, assets[0])
